...
> 不向远端推送任何 binary、不上报任何遥测、不在 `~/.ssh` 写文件。默认情况下，破坏性操作会要求二次确认。
...
| 设置键 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `rss.displayLanguage` | `auto` / `zh-cn` / `en` | `auto` | 插件自有界面与运行时消息的显示语言；修改后需要重新加载窗口。侧栏视图标题会在插件激活后切换；命令、菜单和原生设置标题仍跟随编辑器语言；外部 MCP 工具元数据继续保持英文 |
